...
const SignUpScreen : React.FC<Props> = ({navigation}) => {
    ...
    let validateUser = (): boolean => {
        if(inputName.length <= 2){
            ToastAndroid.showWithGravity("Name should be greater than 2 characters", ToastAndroid.SHORT,ToastAndroid.BOTTOM)
            return false;
            ...
            ToastAndroid.showWithGravity("Password should be grater than 5 characters", ToastAndroid.SHORT,ToastAndroid.BOTTOM)
            return false;
        }else{
            return true;
        }
        ...
        if(validateUser()){
            try{
                ...
                if(users == null){
                    ...
                    if( userDetailsJson.filter((user) => user.email == inputEmail).length > 0 ){
                        ToastAndroid.showWithGravity("User already present!", ToastAndroid.SHORT, ToastAndroid.BOTTOM);
                    }else{
                        ...
                        await AsyncStorage.setItem("@users", JSON.stringify(userDetailsJson));
                        ToastAndroid.showWithGravity("User created successfully!", ToastAndroid.SHORT, ToastAndroid.BOTTOM);
                        navigation.navigate("Login");
                    }
                }
            }catch(error){
                ToastAndroid.showWithGravity("Error while creating a User!", ToastAndroid.SHORT,ToastAndroid.BOTTOM)
            }
        }
